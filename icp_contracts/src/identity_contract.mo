...
actor IdentityContract {
    ...
    // Contract state
    private stable var nextVerificationId: Nat = 1;
    private stable var nextReputationId: Nat = 1;
    
    private var profiles = HashMap.HashMap<Principal, UserProfile>(10, Principal.equal, Principal.hash);
    private var verifications = HashMap.HashMap<Text, VerificationRecord>(50, Text.equal, Text.hash);
    private var userVerifications = HashMap.HashMap<Principal, [Text]>(10, Principal.equal, Principal.hash);
    private var reputations = HashMap.HashMap<Text, ReputationEntry>(100, Text.equal, Text.hash);
    private var userReputations = HashMap.HashMap<Principal, [Text]>(10, Principal.equal, Principal.hash);
    ...
    public query func getPendingVerifications() : async [VerificationRecord] {
        ...
        let pendingVerifications = Array.filter<VerificationRecord>(
            Array.fromIter(allVerifications),
            ...
            func(profile: UserProfile) : Bool {
                ...
                Array.find<Text>(searchSkills, func(searchSkill: Text) : Bool {
                    Array.find<Text>(profile.skills, func(userSkill: Text) : Bool {
                        Text.contains(Text.map(userSkill, Char.toLower), #text Text.map(searchSkill, Char.toLower))
                    }) != null
                }) != null
            }
        );
