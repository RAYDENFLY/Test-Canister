...
export default function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const idUrl = `http://<ii-canister-id>.localhost:4943/#authorize?canisterId=${canister}${originParam}${redirectParam}`;
    // If an environment variable with the II canister id is provided, prefer the local replica address
    const iiCanister = process.env.NEXT_PUBLIC_II_CANISTER_ID || '';
    const localIiUrl = iiCanister ? `http://${iiCanister}.localhost:4943/#authorize?canisterId=${canister}${originParam}${redirectParam}` : idUrl;
    const finalUrl = iiCanister ? localIiUrl : `https://identity.ic0.app/#authorize?canisterId=${canister}${originParam}${redirectParam}`;
